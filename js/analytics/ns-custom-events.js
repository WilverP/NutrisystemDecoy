...
!function() {
/*!****************************************************!*\
  !*** ./resources/js/analytics/ns-custom-events.js ***!
  \****************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _main__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./main */ "./resources/js/analytics/main.js");

window.addEventListener("load", function () {
  console.log("*** Start push omniture to GTM ***");
  var dataLayerEvent = {
    event: 'dataLayerReady'
  };
  /*let dataLayerEvent = GoogleAnalytics.createDataLayerEvent(
      'dataLayerReady',
      null,
      null,
      GoogleAnalytics.getUserId()
  );*/
  ...
  console.log("*** End push omniture to GTM ***"); //
});

window.setPreDataLayer = function () {
  _main__WEBPACK_IMPORTED_MODULE_0__.GoogleAnalytics.triggerPreDataLayer();
};

window.omni_track = function (b) {
  if (typeof dataLayer !== 'undefined') {
    window.dataLayer.push({
      event: 'omniTrackEvent',
      omniTrackEventParam: b
    });
  }
};

window.track_phone_type = function () {
  try {
    var initialPhoneType = $('input[name=phonetype]:visible:checked').val();
    var finalPhoneType = initialPhoneType ? initialPhoneType : $('input[name=phonetype-shortened]:visible:checked').val();

    if (typeof globalClickEventData !== 'undefined' && typeof globalClickEventData.phoneNumberType !== 'undefined') {
      var analyticsVal = finalPhoneType.toLowerCase();
      gtmClickEvent('gaEventLabel=' + analyticsVal, 'phoneNumberType');
    } else {
      console.error('Internal - Missing ns-analytics.js for GTM reporting.');
    }
  } catch (error) {
    console.error('Internal - Unable to report phone type to Google Tag Manager due to Error' + error);
  }
};
/**
 * Global GTM click event data
 */


window.globalClickEventData = {
  clickToCall: {
    "event": "gaTrackEvent-clickToCall",
    "gaEventCategory": "Contact",
    "gaEventAction": "Clicked Phone Number Link",
    "gaEventLabel": ""
  },
  phoneNumberType: {
    "event": "gaTrackEvent-PhoneNumberType",
    "gaEventCategory": "Phone number type",
    "gaEventAction": "Click",
    "gaEventLabel": ""
  }
};

window.gtmClickEvent = function (arg, eventName) {
  console.log("*** Start gtmClickEvent() ***");
  var args = arg.split("|");

  for (var i = 0; i < args.length; i++) {
    var Y = args[i].split("=");
    var gtmAttribute = Y[0];
    var data = Y[1];
    console.log('set ' + gtmAttribute + ' as ' + data + ' in ' + eventName + ' of globalClickEventData object'); //gtmClickEventData is to be rendered through GoogleTagManagerEventDroplet

    if (typeof globalClickEventData !== 'undefined') {
      globalClickEventData[eventName][gtmAttribute] = data;
    }
  }

  ;

  if (typeof dataLayer !== 'undefined' && globalClickEventData[eventName] !== 'undefined') {
    var eventData = globalClickEventData[eventName];
    console.log('push to dataLayer');
    window.dataLayer.push(eventData);
  }

  console.log("*** End gtmClickEvent() ***");
};

window.pageView = function (pageTitle) {
  window.dataLayer.push({
    event: "pageview",
    title: pageTitle,
...
window.gaTrackEvent = function (eventData) {
  dataLayer.push(eventData);
};

window.newMemberGTMEvent = function (eventLabel) {
  dataLayer.push({
    "event": "gaTrackEvent - NewMemberRegisterSuccess",
    "gaEventCategory": "Account Registration",
    "gaEventAction": "New Member Sign-up Submitted",
    "gaEventLabel": eventLabel
  });
};

window.giftCardApplied = function (last4digits, giftCardPin) {
  dataLayer.push({
    event: "gaTrackEvent-eComGiftCardApplied",
    gaEventCategory: "ecommerce",
    gaEventAction: "Gift Card Submitted",
    gaEventLabel: "xxxxxxxxxxxx" + last4digits,
